...
fn main() {
    let matches = App::new("tdz-expandmorph")
        .settings(DEFAULT_CLAP_SETTINGS)
        .arg(Arg::with_name("INPUT").help("Input file").index(1))
        .arg(Arg::with_name("OUTPUT").help("Output file").index(2))
        .arg(
            Arg::with_name("NOPRESERVE")
                .long("no-preserve")
                .short("n")
                .help("do not preserve original TüBa morph tag"),
        )
        .get_matches();
    ...
    for sentence in reader.sentences() {
        ...
        for token in sentence.iter_mut().filter_map(Node::token_mut) {
            expand_tdz_morph(token, !matches.is_present("NOPRESERVE"))
                .or_exit("Cannot expand morphological tag", 1);
        }
